...
const ChatLabel = ({ openMenuId, setOpenMenuId, id, name }) => {
  const { fetchUsersChats, chats, setSelectedChat } = userAppContext()
  ...
  return (
    <div onClick={selectChat} className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
      ...
      >
        {/* Show dots icon only when menu open */}
        <Image src={assets.three_dots} alt='' className={`w-4 ${isOpen ? 'block' : 'hidden'} group-hover:block`} />
